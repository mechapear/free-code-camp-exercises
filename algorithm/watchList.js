...
let ratings = watchList.map((list) => {
  return {
    title: list.Title,
...
const newList = watchList.map((list) => {
  return {
    title: list.Title,
    rating: list.imdbRating,
  }
...
const filteredList = newList.filter((item) => item.rating >= 8.0)
...
// the rating values are saved as strings in the object and need to be converted into numbers before they are used in any mathematical operations.
function getNolanList(watchList) {
  return watchList
...
function getRating(watchList) {
  const nolanList = getNolanList(watchList)
  const sumRating = nolanList.reduce(
    (sum, nolanList) => sum + Number(nolanList.rating),
    0
  )
  const averageRating = sumRating / nolanList.length
